...
namespace Event_Management_System.API.Application.Implementation
{
    public class BookingService : IBookingService
    {
        ...
        public async Task<APIResponse<CreateBookingResponseDto>> CreateBooking(Guid userId, CreateBookingDto createBookingDto)
        {
            ...
            if (user == null)
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.BadRequest, "User does not exist", null);
            ...
            if (eventCentre == null)
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.NotFound, "Event center does not exist", null, new Error { Message = "Event center does not exist" });

            if (!eventCentre.IsActive)
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.BadRequest, "Event center is not active", null, new Error { Message = "Event center is not active" });

            if (!eventCentre.Availabilities.Any())
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.BadRequest, "Event center has no availability periods defined", null, new Error { Message = "Event center has no availability periods defined" });

            // Validate booking falls within event center availability periods defined
            ...
            if (!availabilityValidation.IsValid)
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.BadRequest, availabilityValidation.ErrorMessage, null, new Error { Message = availabilityValidation.ErrorMessage });

            // Check for booking conflicts
            var isAvailable = await CheckEventCentreAvailabilityAsync(createBookingDto.EventCentreId, createBookingDto.BookedFrom, createBookingDto.BookedTo);
            if (!isAvailable)
            {
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.BadRequest, "The event center is not available for the specified date and time", null, new Error { Message = "The event centre is not available for the specified date and time" });
            }
            ...
            try
            {
                var isPaidBooking = eventCentre.PricePerDay > 0;
                ...
                var bookingRecord = new Booking
                {
                    ...
                    CreatedDate = now,
                    ModifiedDate = now,
                    BookingReservationExpiresAt = isPaidBooking ? now.AddMinutes(5) : null,
                ...
                var numberOfDays = (int)Math.Ceiling((createBookingDto.BookedTo - createBookingDto.BookedFrom).TotalDays);
                if (numberOfDays < 1) numberOfDays = 1;
                var totalAmount = eventCentre.PricePerDay * numberOfDays;

                // If the event centre has a price, initiate Paystack payment
                if (totalAmount > 0)
                {
                    var initiatePaymentDto = new InitiatePaymentDto
                    {
                        ...
                        Amount = totalAmount,
                        Currency = "NGN",
                        Description = $"Payment for booking at {eventCentre.Name} from {bookingRecord.BookedFrom:yyyy-MM-dd} to {bookingRecord.BookedTo:yyyy-MM-dd}",
                        PaymentType = PaymentType.Booking,
                        ReferenceId = bookingRecord.Id,
                        Metadata = new Dictionary<string, string>
                        {
                            { "booking_id", bookingRecord.Id.ToString() },
                            { "event_centre_id", eventCentre.Id.ToString() },
                            { "event_centre_name", eventCentre.Name },
                            { "booked_from", bookingRecord.BookedFrom.ToString("o") },
                            { "booked_to", bookingRecord.BookedTo.ToString("o") },
                    ...
                    {
                        _logger.LogError("Payment initialization failed for booking {BookingId}", bookingRecord.Id);
                        await transaction.RollbackAsync();

                        return APIResponse<CreateBookingResponseDto>.Create(
                            HttpStatusCode.InternalServerError,
                            "An error occurred while initiating payment for the booking",
                            null,
                            new Error { Message = "Payment initialization failed. Booking has been cancelled." });
                    }
                    bookingRecord.BookingStatus = BookingStatus.PendingPayment;
                     _dbContext.Bookings.Update(bookingRecord);
                    await _dbContext.SaveChangesAsync();

                    await transaction.CommitAsync();

                    var paidResponse = new CreateBookingResponseDto
                    {
                        BookingId = bookingRecord.Id,
                        EventCentreId = eventCentre.Id,
                        EventCentreName = eventCentre.Name,
                        BookedFrom = bookingRecord.BookedFrom,
                        BookedTo = bookingRecord.BookedTo,
                        BookingStatus = bookingRecord.BookingStatus.GetEnumDescription(),
                        TotalAmount = totalAmount,
                        PaymentUrl = paymentResult.Data.PaymentUrl,
                        PaymentReference = paymentResult.Data.TransactionReference,
                        CreatedDate = bookingRecord.CreatedDate
                    };
                ...
                return APIResponse<CreateBookingResponseDto>.Create(HttpStatusCode.Created, "Booking created successfully", freeResponse);
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync();
        ...
        public async Task<APIResponse<PagedList<BookingDto>>> GetAllBookings(Guid userId, RequestParameters requestParameters, BookingFilter bookingFilter)
        {
            ...
            if (bookingFilter.EventCentreId.HasValue)
            {
        ...
        public async Task<APIResponse<object>> UpdateBookingStatus(Guid userId, BookingStatus bookingStatus, Guid bookingId)
        {
            ...
            if (booking.BookingStatus == BookingStatus.Cancelled || booking.BookingStatus == BookingStatus.Rejected || booking.BookingStatus == BookingStatus.Expired)
                return APIResponse<object>.Create(HttpStatusCode.BadRequest, "Cannot update a cancelled, rejected or expired booking", null, new Error { Message = "Cannot update a cancelled, rejected or expired booking" });
        ...
        {
            // 1. Input validation
            if (bookedFrom <= DateTime.UtcNow)
                return APIResponse<object>.Create(
                    HttpStatusCode.BadRequest,
                    "Booking start time must be in the future",
                    null,
                    new Error { Message = "Booking start time must be in the future" });
            ...
            if (bookedTo <= bookedFrom)
                return APIResponse<object>.Create(
                    ...
                    null,
                    new Error { Message = "Booking end time must be after start time" });

            // 2. Get IDs of event centres that are booked during the specified time range
            ...
            // 3. Get active event centres that are not booked and have availabilities defined
            var candidateEventCentres = await _dbContext.EventCentres
                .Include(ec => ec.Availabilities)
                .Where(ec => ec.IsActive 
                          && !bookedEventCentreIds.Contains(ec.Id)
                          && ec.Availabilities.Any())  // Only get centres with availabilities
                .ToListAsync();

            // 4. Filter by availability validation (done in-memory for complex logic)
            var availableEventCentres = candidateEventCentres
                .Where(ec => ValidateBookingAgainstEventCentreAvailability(
                    bookedFrom, 
                    bookedTo, 
                    ec.Availabilities.ToList()).IsValid)
                .ToList();
            ...
            // 5. Map to DTOs
            var response = availableEventCentres.Select(ec => new EventCentreDto
            {
                Id = ec.Id,
                Name = ec.Name,
        ...
        private async Task<bool> CheckEventCentreAvailabilityAsync(Guid eventCentreId, DateTime bookedFrom, DateTime bookedTo)
        {
            bool hasConflict = await _dbContext.Bookings
                .Where(b => b.BookingStatus != BookingStatus.Cancelled && b.BookingStatus != BookingStatus.Rejected && b.BookingStatus != BookingStatus.Expired)
                .AnyAsync(b => b.EventCentreId == eventCentreId
                       && b.BookedFrom < bookedTo
                       && b.BookedTo > bookedFrom);
