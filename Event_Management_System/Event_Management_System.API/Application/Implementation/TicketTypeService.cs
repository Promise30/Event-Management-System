...
namespace Event_Management_System.API.Application.Implementation
{
    public class TicketTypeService : ITicketTypeService
    {
        ...
        public async Task<APIResponse<object>> CreateTicketTypeAsync(Guid userId, CreateTicketTypeDto createTicketTypeDto)
        {
            ...
            // check if total capacity does not exceed event centre capacity
            if (createTicketTypeDto.Capacity > eventEntity.Booking.EventCentre.Capacity)
            {
                return APIResponse<object>.Create(HttpStatusCode.BadRequest, "Total capacity exceeds event centre capacity", null, new Error { Message = "Total capacity exceeds event centre capacity" });
            }

            // check if ticket type with the same name already exists for the event
            var existingTicketType = await _dbContext.TicketTypes.AsNoTracking().FirstOrDefaultAsync(tt => tt.EventId == createTicketTypeDto.EventId && tt.Name.ToLower() == createTicketTypeDto.Name.ToLower());
        ...
        public async Task<APIResponse<PagedResponse<List<TicketTypeDto>>>> GetAllTicketTypesAsync(RequestParameters requestParameters, SortParameters? sortParameters, string? searchTerm, Guid eventId)
        {
            ...
            if (!string.IsNullOrEmpty(searchTerm))
            {
               // searchTerm = searchTerm.Trim().ToLower();
                ticketTypesQuery = ticketTypesQuery.Where(tt => EF.Functions.Like(tt.Name, $"%{searchTerm}%")
                                              || EF.Functions.Like(tt.Description ?? "", $"%{searchTerm}%"));
            ...
            var ticketTypes = await _databaseRepository.GetAllPaginatedAsync( ticketTypesQuery, requestParameters);
